/**
 * An implementation of a VigenereCipher where the given word will either be 
 * encoded or decoded with the given key
 * 
 * @author Alma Ordaz
 * 
 */
...
public class VigenereCipher {
  ...
  // PostConditions: the result of adding the two chars together will be returned
  // as a char. 'a' will be associated with the number 0 and each lowercase letter will follow
  public static char charAdd(char text, char key) {
    int base = (int) 'a';
    int result = ((text - base) + (key - base)) % ALPHABET_LENGTH;

    return (char) (result + base);
  }// charAdd

  // PreConditions: the unputed chars are lowecase letters a-z
  // PostConditions: the result of subtracting the two chars together will be
  // returned as a char 'a' will be associated with the number 0 and each 
  // lowercase letter will follow
  ...
  // PreConditions: none
  // PostConditions: text will be decoded or encoded using key and returned as a
  // string
